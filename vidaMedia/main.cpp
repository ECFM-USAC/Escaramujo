...
//	TH1F *tiempoDecaimiento = new TH1F ("tiempoDecaimiento", "Tiempo de Decaimiento; numero de hits; entradas",50,0,3e3);
	/* Nombre objeto              nombre objeto, titulo, n Bines, mínimo, máximo */

	/// Aqui empecé yo

	int ventana = 50; //Nanosegundos
	int contadorFiltro = 0; 
	int veto = 465; //Nanosegundos
	ofstream salida;
	salida.open("filtro.dat");
...
//		hPulse_ch0->Fill(sizeTDC_0);
     
		///////////////////////////////////////////////////

		if (sizeTDC_0==1 && sizeTDC_1==1 && sizeTDC_2==1) 
		{
			//if ( fabs(TDC_LE_2[0]- TDC_LE_0[0]) <= ventana && (TDC_LE_2[0]- TDC_LE_0[0]) >= 0 )
			if ( fabs(TDC_LE_2[0]- TDC_LE_0[0]) <= ventana )
			{
				//if ( fabs(TDC_LE_1[0]- TDC_LE_2[0]) >= veto &&  (TDC_LE_1[0]- TDC_LE_2[0]) >= 0 )
				if ( fabs(TDC_LE_1[0]- TDC_LE_2[0]) >= veto )
				{
//					tiempoDecaimiento->Fill( TDC_LE_1[0] - TDC_LE_2[0] );
					salida << TDC_LE_1[0] - TDC_LE_2[0] << endl ;
					contadorFiltro++;
				}
			}
		}
...
//		TCanvas *c1 = new TCanvas("c1","c1");//Declaración del canvas
//		c1->cd();//acceder al canvas

		double porcentaje = static_cast <double> (contadorFiltro) / n_events ;

		cout << porcentaje << endl;

//		tiempoDecaimiento->Draw();
		salida.close();
  }
